...
// Hero is injected via script AFTER Next.js hydration to avoid DOM wipe.
...
/* Hide original hero (CSS bracket escaping) */
...
#dlg-hero { position: relative; width: 100%; min-height: 100vh; display: flex; flex-direction: column; background: #0a1628; font-family: 'Inter','Helvetica Neue',Arial,sans-serif; color: #fff; overflow: hidden; }
#dlg-bg { position: absolute; inset: 0; background: url('${BG}') center/cover no-repeat; z-index: 1; }
#dlg-overlay { position: absolute; inset: 0; background: linear-gradient(92deg,rgba(5,15,30,.92) 0%,rgba(5,15,30,.65) 48%,rgba(5,15,30,.18) 100%),linear-gradient(180deg,rgba(0,0,0,.50) 0%,rgba(0,0,0,0) 35%,rgba(0,0,0,.44) 100%); z-index: 2; }
#dlg-nav { position: relative; z-index: 20; display: flex; align-items: center; justify-content: space-between; padding: 24px 56px; gap: 24px; }
...
#dlg-mob-menu { display: none; position: absolute; top: 0; left: 0; right: 0; background: rgba(5,15,30,.97); padding: 76px 28px 36px; z-index: 25; flex-direction: column; }
...
.dlg-cta { display: inline-flex; align-items: center; gap: 10px; background: #f5b700; color: #0a1628; padding: 16px 32px; border-radius: 6px; font-weight: 700; font-size: 15px; letter-spacing: .5px; width: fit-content; box-shadow: 0 8px 24px rgba(245,183,0,.30); transition: transform .2s, box-shadow .2s; }
.dlg-cta:hover { transform: translateY(-2px); box-shadow: 0 12px 32px rgba(245,183,0,.46); }
#dlg-proof { position: relative; z-index: 5; margin-top: 48px; background: linear-gradient(180deg,rgba(5,15,30,0) 0%,rgba(5,15,30,.88) 100%); padding: 28px 56px 38px; }
#dlg-proof-inner { max-width: 1180px; margin: 0 auto; display: grid; grid-template-columns: repeat(4,1fr); gap: 24px; text-align: center; }
...
@media (max-width: 1024px) { #dlg-nav { padding: 20px 32px; } #dlg-content { padding: 80px 32px 0; } #dlg-proof { padding: 24px 32px 32px; } }
@media (max-width: 767px) { #dlg-nav-links, #dlg-explore-btn { display: none; } #dlg-burger { display: flex; } #dlg-nav { padding: 18px 20px; } #dlg-content { padding: 44px 20px 0; justify-content: flex-start; } .dlg-sub { margin-bottom: 28px; } .dlg-cta { padding: 14px 26px; font-size: 14px; } #dlg-proof { padding: 20px 20px 32px; margin-top: 36px; } #dlg-proof-inner { grid-template-columns: repeat(2,1fr); gap: 20px 12px; } .dlg-stat-big { font-size: 22px; } }
@media (max-width: 380px) { .dlg-stat-big { font-size: 19px; } }
</style>`;

// The hero HTML is a JS string, injected into the page as a script that runs
// AFTER Next.js hydration — so React cannot wipe our DOM nodes.
const INJECT_SCRIPT = `<script id="dlg-inject">(function(){
var H = '<section id="dlg-hero" aria-label="Hero">'
  ...
  +     '<img src="/_next/image?url=%2Fimages%2Flogo.png&w=384&q=75" alt="Diligram" height="36" width="108" loading="eager">'
  +   '</a>'
  ...
  +   '<p class="dlg-sub">The award-winning AI-driven platform giving leaders real-time visibility into who has received, engaged with, and acted on critical information &mdash; across every frontline.</p>'
...
// Run after load event + 300ms to ensure Next.js hydration is complete
...
export default async (request, context) => {
  ...
  // 1. Update page title in HTML (for SEO/crawlers)
  ...
  // 3. Inject the post-hydration script before </body>
